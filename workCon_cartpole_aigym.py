...
def simulate_cartpole_batch(m_c, m_1, l_1, y0=None,
                           ...
                           device='cuda:0', test_mode={'on': False, 'context': 50}):
    """
    ...
    for i in range(num_steps - 1):
        ...
        if i < 50 and not test_mode['on']:
            u = torch.zeros(batch_size, device=device)  # no control for first 50 steps to test open-loop swing-up
            # mode = torch.zeros_like(u)
            mode = torch.ones_like(u)*-1  # use -1 to indicate open-loop mode
        elif i < test_mode['context'] and test_mode['on']:
            u = torch.zeros(batch_size, device=device)  # no control for context steps in test mode
            mode = torch.ones_like(u)*-1  # use -1 to indicate open-loop mode
        else:
        ...
        if not test_mode['on']:
            noise = torch.randn_like(y_next) * 0.02 #0.03  # small noise
        else:
            noise = torch.zeros_like(y_next)
...
def checking(Y0, total_time, m_c=torch.tensor([1.0]), m_1=torch.tensor([1.0]), l_1=torch.tensor([1.0]), method='rk4', dt=0.025, device='cuda:0', test_mode={'on': False, 'context': 50}):
    """
    ...
    """
    if method == 'rk4':
        t, y, control_logger, control_mode_logger = simulate_cartpole_batch(m_c=m_c, m_1=m_1, l_1=l_1, y0=Y0, total_time=total_time, dt=dt, device=device, test_mode=test_mode)
